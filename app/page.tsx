...
const tools = [
  ...
  ["/salary-hike-calculator","Salary Hike Calculator","See how a raise changes your annual and monthly pay.","📈"],
  ...
  ["/loan-prepayment-calculator","Loan Prepayment","Estimate interest and tenure savings from prepayment.","💳"],
  ["/sip-calculator","SIP Calculator","Estimate corpus from a monthly SIP.","📊"],
  ["/step-up-sip-calculator","Step-Up SIP","See how annual SIP increases can affect your goal.","🚀"],
  ["/1-crore-calculator","₹1 Crore Goal","Estimate the SIP needed for a ₹1 crore target.","💎"],
  ["/retirement-calculator","Retirement Calculator","Estimate your retirement corpus and funding gap.","🌅"],
  ["/job-offer-comparison","Job Offer Comparison","Compare two job offers beyond headline CTC.","⭐"],
  ["/financial-health","Financial Health Score","Get a transparent 0–100 money-health snapshot.","❤️"]
] as const;
...
export default function Home() {
  return <main>
      ...
      <div className="container py-16 md:py-24 grid lg:grid-cols-[1.2fr_.8fr] gap-12 items-center">
        <div>
          ...
          <div className="grid grid-cols-3 gap-3 mt-10 max-w-xl">
            ...
            <div className="card p-4"><b>12+</b><div className="text-xs muted mt-1">MVP tools</div></div>
        ...
        <div className="card p-7 bg-slate-950 text-white">
          ...
          <div className="mt-6 space-y-3">
            {["My in-hand salary","Income tax","Home loan EMI","SIP for ₹1 crore","Retirement corpus"].map((x)=><Link key={x} href="#tools" className="block rounded-xl bg-white/10 hover:bg-white/15 px-4 py-3 text-sm">{x} <span className="float-right">→</span></Link>)}
          </div>
    ...
    <section id="tools" className="container py-16">
      <div className="max-w-2xl"><div className="text-blue-700 font-bold text-sm">FINANCE TOOLKIT</div><h2 className="text-3xl md:text-4xl font-black mt-2">Tools that explain the number.</h2><p className="muted mt-3">Every calculator is designed to show the result, breakdown, assumptions and what it means for you.</p></div>
    ...
    <section className="container pb-16"><div className="card p-8 md:p-10 bg-gradient-to-br from-blue-50 to-teal-50"><div className="max-w-3xl"><div className="text-sm font-bold text-teal-700">WHY NEXZY MONEY</div><h2 className="text-3xl font-black mt-2">A calculator should answer more than “what is the number?”</h2><p className="text-slate-600 mt-4 leading-7">Nexzy Money is designed as a financial decision platform: calculate, understand, compare and plan. Tax and investment estimates are clearly labelled, with assumptions and update dates.</p></div></div></section>
  </main>
}
